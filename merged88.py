...
# git history. Set these in a local .env file (gitignored) or in your
# shell/service environment before running the app.
...
SQLITE_DB = os.path.join(BASE_DIR, "shared_data.db")
YOLO_MODEL = os.path.join(BASE_DIR, "bestold.pt")
YOLO_CLS_NAMES = ["frontface", "endface"]
...
# IMPORTANT: all of these used to be bare relative paths ("reports",
# "cut_events", ...), which resolve relative to whatever the CURRENT
# WORKING DIRECTORY happens to be when the process is started - not
# necessarily the folder this script lives in. If you launch this via a
# shortcut, a scheduled task/service, a different terminal cwd, or any
# launcher other than "cd into this folder and run it", Python silently
# creates a brand-new, empty "reports" folder wherever the cwd happened to
# be, and the dashboard reads/writes THAT empty folder - which looks
# exactly like "the dashboard is blank" even though your real data (if any)
# is sitting untouched in reports/ next to the script. Anchoring every path
# to BASE_DIR makes this always resolve to the same folder next to the
# script, no matter how/where it's launched from.
...
# ── REPORT_DIR resolution (fixes "dashboard shows no previous records") ──
# Prefer BASE_DIR/reports (robust, launcher-independent). But if that one
# is empty AND a legacy cwd-relative "reports" folder actually holds real
# records (because this instance used to be launched from a folder other
# than the script's own), fall back to that folder so old records show up
# again, and say so loudly on startup instead of failing silently.
_ANCHORED_REPORT_DIR = os.path.join(BASE_DIR, "reports")
_LEGACY_REPORT_DIR = os.path.abspath("reports")

def _has_report_data(folder: str) -> bool:
    """True if this folder has a state file or a CSV with more than just a header row."""
    try:
        if os.path.isfile(os.path.join(folder, "current_running_session.json")):
            return True
        for fname in ("blade_reset_report.csv", "item_wise_reset_report.csv"):
...
if not _has_report_data(_ANCHORED_REPORT_DIR) and _LEGACY_REPORT_DIR != _ANCHORED_REPORT_DIR \
        and _has_report_data(_LEGACY_REPORT_DIR):
    REPORT_DIR = _LEGACY_REPORT_DIR
    print(f"[REPORTS] No data in {_ANCHORED_REPORT_DIR}; found existing records in "
          f"{_LEGACY_REPORT_DIR} instead - using that folder so previous records show up.")
else:
    REPORT_DIR = _ANCHORED_REPORT_DIR
os.makedirs(REPORT_DIR, exist_ok=True)
print(f"[REPORTS] Using report folder: {REPORT_DIR}")
    ...
    try:
        with open(path, "r", newline="", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                # A row with MORE fields than the header (e.g. two rows that
                # got written to the same physical CSV line without a
                # newline between them - which is exactly what corrupted
                # blade_reset_report.csv previously) makes DictReader stash
                # the extra values under a literal `None` key. That's a
                # dict with a None key mixed in with string keys, and
                # Flask's JSON encoder (sort_keys=True by default) crashes
                # with "'<' not supported between NoneType and str" the
                # moment it tries to sort those keys - taking down the
                # WHOLE /api/blade_logs (or /api/item_logs) response, not
                # just that one bad row. Drop the extra/None-keyed data and
                # log a warning instead of letting one malformed row 500
                # every other row too.
                if None in row:
...
# NOTE: sync_excel() is no longer called anywhere. It used to run on every
# blade cut / item update to keep BLADE_XLSX_PATH / ITEM_XLSX_PATH mirrored,
# but neither file is ever served by a Flask route, so those calls were pure
# overhead sitting inside the video-detection hot loop. Kept here (unused)
# in case a future feature wants a live per-sheet xlsx mirror again.
...
def build_combined_excel() -> str:
    ...
    # Save atomically (temp file + rename) so a request that reads the file
    # while it's mid-write (e.g. a second, overlapping download click)
    # never sees a half-written/corrupt xlsx.
    tmp_path = COMBINED_XLSX_PATH + ".tmp"
    wb.save(tmp_path)
    os.replace(tmp_path, COMBINED_XLSX_PATH)
...
def append_blade_report(blade_count: int, blade_start: str, current_item: str, item_breakdown: Dict[str, int]) -> Dict[str, Any]:
    ...
    # NOTE: BLADE_XLSX_PATH is never served by any route (only the combined
    # workbook built on-demand in build_combined_excel() is downloaded), so
    # rebuilding it here on every blade reset was pure wasted disk/CPU work.
    return {
...
def create_new_item_session_row() -> Dict[str, Any]:
    ...
    with _csv_lock:
        ...
        with open(ITEM_CSV_PATH, "a", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow(row)
    # NOTE: ITEM_XLSX_PATH is never served by any route - removed the
    # per-event xlsx rebuild here too (see comment in append_blade_report).
...
def update_item_session(item_name: str, count: int) -> None:
    ...
    with _csv_lock:
        ...
        if len(rows) < 2:
            ...
            # (create_new_item_session_row also takes _csv_lock, which is
            # fine: Python's threading.Lock here would deadlock on
            # re-entry, so call the unlocked body directly instead.)
            rid = next_id(ITEM_CSV_PATH, ITEM_HEADERS)
    ...
    # NOTE: this function runs on EVERY blade cut (called from the
    # detection_loop hot path). It previously also called sync_excel(),
    # which reopens the whole CSV, rebuilds an openpyxl Workbook from
    # scratch, recomputes every column width, and writes item_wise
    # report.xlsx to disk - on every single cut. That xlsx file is never
    # served by any Flask route (only the combined workbook built fresh
    # in build_combined_excel() is downloaded), so this was pure wasted
    # CPU/disk I/O sitting inside the video-detection loop on every cut.
    # It was the main cause of the dashboard lagging/appearing stale and
    # of "server overload" errors when downloading the combined Excel
    # while cuts were happening. Removed - CSV write above is all that's
    # needed; the xlsx is generated on demand at download time.
...
_csv_lock = threading.Lock()  # guards every write to the report CSVs so two
                               # near-simultaneous blade/item events (e.g. an
                               # auto-reset firing at the same moment as a
                               # manual button click) can never interleave
                               # their writes onto the same physical CSV
                               # line - that corruption is what caused a
...
def gstreamer_loop() -> None:
    ...
    print(f"[GST] Looking for YOLO weights at: {YOLO_MODEL}")
    if not os.path.isfile(YOLO_MODEL):
        yolo_error = f"weights file not found at {YOLO_MODEL}"
        ...
        try:
            ...
            yolo_available = True
            print(f"[GST] YOLO model loaded. Classes in model: {model.names}")
        except Exception as e:
            import traceback
            yolo_error = f"{type(e).__name__}: {e}"
            print(f"[GST] YOLO load failed ({yolo_error}). Streaming raw frames.")
            traceback.print_exc()
    ...
    def _annotate(frm):
        ...
        try:
            ...
            for i in range(n):
                ...
                cls_label = YOLO_CLS_NAMES[clsid] if clsid < len(YOLO_CLS_NAMES) else f"cls{clsid}"
...
def download_combined_excel():
    try:
        ...
        print(f"[ERROR] build_combined_excel failed: {e}")
        return jsonify({"status": "error", "message": f"Could not build Excel report: {e}"}), 500
    try:
        return send_file(path, as_attachment=True, download_name="msp_hotsow_combined_report.xlsx")
    except Exception as e:
        print(f"[ERROR] send_file failed for combined excel: {e}")
        return jsonify({"status": "error", "message": f"Could not send Excel file: {e}"}), 500
...
<title>MSP Hotsow 2 – Cut Counter</title>
...
<header class="header">
  <div class="header-left">
    ...
    <div>
      <div class="header-title">HOTSOW 2 – Cut Counter</div>
...
function connectSSE() {
  ...
  es.addEventListener('cut', e => {
    ...
    renderState();
    loadLogs();   // was missing: without this the log tables never
                  // refreshed after a cut, only after a reset/change,
                  // so they looked "blank"/frozen even while blade_count
                  // kept incrementing live.
  });
...
if __name__ == "__main__":
    ...
    print("=" * 64)
    print("MSP Blade + Item Wise Cut Counter  v3")
    print(f"Report folder  : {REPORT_DIR}")
    print(f"Blade CSV file : {BLADE_CSV_PATH}  "
          f"({len(read_csv_logs(BLADE_CSV_PATH, BLADE_HEADERS))} rows)")
    print(f"Item CSV file  : {ITEM_CSV_PATH}  "
          f"({len(read_csv_logs(ITEM_CSV_PATH, ITEM_HEADERS))} rows)")
    print("Main Dashboard : http://localhost:5025/")
    print("Vision Feed    : http://localhost:5025/vision")
    print("Blade CSV      : http://localhost:5025/download/blade_csv")
    print("Item CSV       : http://localhost:5025/download/item_csv")
    print("Combined Excel : http://localhost:5025/download/combined_excel")
    print("Live Blade CSV : http://localhost:5025/download/blade_live_csv")
    print("Live Item CSV  : http://localhost:5025/download/item_live_csv")
    print("=" * 64)
    app.run(host="0.0.0.0", port=5025, debug=False, use_reloader=False, threaded=True)